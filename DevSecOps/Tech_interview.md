...
**Q** Что такое "подсоленный хэш"? </br>
**A** «Подсоленный хэш» — это хэш пароля (или других данных), к которому добавлена случайная строка — “соль” (salt) перед вычислением хэша. </br>
! Соль не должна быть секретной</br>
Она должна быть достаточно длинной и случайной</br>
Лучше использовать специальные алгоритмы:</br>
- bcrypt
- scrypt
- Argon2 (
</details>
...
**Q** каким образом работает подписание коммита в git??<br>
**A** Подписание коммита в Git — это криптографическая подпись содержимого коммита (хэша), обычно с помощью GPG или SSH-ключа. <br>
Git подписывает весь объект коммита приватным ключом и сохраняет подпись внутри коммита.√
При проверке используется публичный ключ: если содержимое коммита изменено или ключ не совпадает, подпись становится невалидной.<br>
Это гарантирует авторство и целостность коммита, но не доверие к самому автору.

**Q** Что такое Gpg?<br>
**A** GPG — это реализация стандарта OpenPGP, предназначенная для шифрования данных и создания цифровых подписей. Это инструмент для шифрования и цифровой подписи, который Git использует для подтверждения авторства и неизменности коммитов.<br>

**Q** Что такое rebase и для чего он нужен?<br>
**A** Rebase в Git — это операция, которая переписывает историю коммитов, перенося коммиты одной ветки поверх другой. <br>

**Q** Что такое тэг и для чего он нужен?<br>
**A** Tag — это неизменяемая метка на коммите, чаще всего используемая для обозначения релизов.<br>

**Q** Каким образом изменить protected branch из терминала?<br>
**A** Protected branch — это настройка сервера, и изменить её можно только через UI или API, но не через обычные git-команды.<br>

**Q** Имеете ли опыт работы с рисками Информационной безопасности?<br>
**A** “Да, у меня есть опыт работы с оценкой рисков ИБ в проектах, включая выявление угроз и их классификацию.”<br>
“Для системного анализа рисков я использовал методологию STRIDE, которая позволяет классифицировать угрозы по 6 категориям: Spoofing, Tampering, Repudiation, Information disclosure, Denial of service, Elevation of privilege.” <br>
Если кратко, при разработке движка казино я провел STRIDE-анализ, выявил потенциальные угрозы доступа и модификации данных, после чего совместно с командой внедрил контролы аутентификации, журналирования и шифрования, что снизило риски на ранней стадии.”<br>

**Q** Что отражает некоректность контролей ИБ?<br>
**A** это когда предусмотренные механизмы защиты не способны реально предотвратить, обнаружить или смягчить угрозы.<br>
<br>
проявляться например как:<br>
Неправильная настройка – например, слабые пароли, открытые порты, некорректные права доступа.<br>
Неадекватные политики и процедуры – политики ИБ устарели или не соответствуют текущим рискам.<br>
Технические уязвимости – баги в системе, позволяющие обходить контроль.<br>
Отсутствие мониторинга или реагирования – нет отслеживания события или реакции на инциденты.<br>
Несоответствие compliance – не покрывает требования регулятора (например, HIPPA, GDPR).<br>

**Q** Какие методы работы с рисками в ИБ существуют?<br>
**A** Методы работы с рисками ИБ включают: идентификацию и оценку угроз, снижение рисков с помощью контролей, <br> принятие или удержание допустимых рисков, передачу рисков третьим сторонам (например, через страхование), и мониторинг эффективности мер.<br>

**Q** Какая класификация рисков в ИБ?<br>
**A** Риски ИБ классифицируются по источнику угроз (внутренние/внешние), типу воздействия на данные (конфиденциальность, <br> целостность, доступность), вероятности и уровню ущерба, а также по природе угрозы (технические, организационные, природные).

**Q** какие требования fintech реализовывали? <br>
**A** В Fintech реализовывал безопасную облачную инфраструктуру на AWS/Kubernetes, автоматизированные CI/CD с SAST/DAST, <br> управление секретами (Vault, Secrets Manager), мониторинг и SIEM (Grafana, Loki, Wazuh), защиту Kubernetes и приложений (Kyverno, Falco, Cloudflare), <br> а также бэкапы и disaster recovery.<br>


**Q** Умеете ли организовывать triage?<br>
**A** Triage — это процесс приоритизации и классификации инцидентов или уязвимостей по степени риска, влияния и срочности.

**Q**  Что такое finding? <br>
**A** Finding — это выявленная проблема или уязвимость в ходе тестирования безопасности или аудита<br>
Пример: уязвимость SQL-инъекции, некорректные права доступа, открытые порты.<br>

**Q**Что такое fallse positive?<br>
**A** когда система или тест сообщает о проблеме, которой на самом деле нет.
Пример: сканер SAST отмечает безопасный код как уязвимы<br>

**Q**Что такое false negative? <br>
**A** False negative (ложное отрицание) — когда реальная проблема не выявляется системой безопасности или тестом.<br>
Пример: уязвимость SQL-инъекции не обнаружена сканером.<br>

**Q** какие security misconfiguration существеют?<br>
**A** Security misconfiguration — это ошибки конфигурации систем, приложений или облачных сервисов, создающие уязвимости, например <br>
открытые порты, небезопасные права доступа, неправильные IAM-политики, небезопасные TLS-настройки или debug-режимы в продакшене.

**Q** какие коды ошибок наиболее критичнв при обнорудении пектора атаки?<br>
**A** Наиболее критичные коды ошибок при выявлении векторов атаки — это 5xx (сервера) и 4xx, раскрывающие контроль <br>
доступа и существование ресурсов. Особенно опасны, если сервер возвращает подробные сообщения об ошибках, включая стек-трейсы или конфигурацию.<br>
